...
interface KanvasEdit {
  label: string;
  before: KanvasScene;
  after: KanvasScene;
}

function emptyScene(): KanvasScene {
  return { elements: [], appState: {} };
}

export class KanvasDocument implements vscode.CustomDocument {
  static async create(
    uri: vscode.Uri,
    backupId: string | undefined,
    onWatcherChange: (document: KanvasDocument) => void
  ): Promise<KanvasDocument> {
    const dataUri = backupId ? vscode.Uri.parse(backupId) : uri;
    const scene = await KanvasDocument.readFile(dataUri);
    const doc = new KanvasDocument(uri, scene);
    doc.startWatcher(onWatcherChange);
    return doc;
  }

  public static async readFile(uri: vscode.Uri): Promise<KanvasScene> {
  ...
  private _scene: KanvasScene;
  private _edits: KanvasEdit[] = [];
  ...
  private readonly _onDidChangeSceneForWebview = new vscode.EventEmitter<KanvasScene>();
  public readonly onDidChangeSceneForWebview = this._onDidChangeSceneForWebview.event;

  private constructor(uri: vscode.Uri, scene: KanvasScene) {
    this._uri = uri;
    this._scene = scene;
    this._savedSceneJson = JSON.stringify(scene);
  }

  private startWatcher(onWatcherChange: (document: KanvasDocument) => void) {
  ...
  makeEdit(newScene: KanvasScene, label = 'Edit'): void {
  ...
  async revert(): Promise<void> {
    const scene = await KanvasDocument.readFile(this._uri);
    this._scene = scene;
    this._savedSceneJson = JSON.stringify(scene);
